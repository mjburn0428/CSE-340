...
validate.messageRules = () => {
  return [body("message_to").trim().isNumeric().withMessage('Please select a recipient'),

    body("message_subject").trim().isLength({ min: 3 }).withMessage("Subject must be longer than 3 characters"),

    body("message_body").trim().isLength({ min: 1 }).withMessage("Please include a message"),
  ]
}
...
validate.checkMessageData = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    res.render("./inbox/send", {
      errors,
      title: "New Message",nav,accountSelect, message_from, message_subject, message_body
    })
    return
  }
  next()
...
validate.replyRules = () => {
  return [
    body("reply_message").trim().isLength({ min: 1 })
